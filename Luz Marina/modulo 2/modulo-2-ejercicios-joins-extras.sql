...
SELECT DISTINCT cu.company_name AS NombreEmpresa, YEAR(ord.order_id) AS Año, COUNT(orc.product_id) AS NumObjetos
FROM orders as ord
LEFT JOIN order_details AS orc
...
SELECT  cu.company_name AS NombreEmpresa, ord.order_date AS Fechapedido, ord.order_id AS PedidosRealizados
FROM orders as ord
INNER JOIN order_details AS orc
ON ord.order_id = orc.order_id
...
SELECT ca.category_name AS NombreCategoria, pr.product_name AS NombreProducto, ROUND(SUM(orc.quantity * orc.unit_price *( 1 - orc.discount) ),2) AS PrecioPedido
FROM products AS pr
INNER JOIN categories AS ca
ON ca.category_id = pr.category_id
INNER JOIN order_details AS orc
ON orc.product_id = pr.product_id
...
-- del propio Reino Unido de cara a conocerlos mejor y poder adaptarse al mercado actual. Especificamente nos piden el nombre de 
-- cada compañía cliente junto con el número de pedidos.
...
-- También nos han pedido que obtengamos todos los nombres de las empresas cliente de Reino Unido (tengan pedidos o no) junto con 
-- los ID de todos los pedidos que han realizado y la fecha del pedido.
...
ON cu.customer_id = ord.customer_id;
WHERE country = "UK"
GROUP BY cu.company_name, ord.order_id;
...
SELECT a.city AS CiudadEmpleado, a.first_name AS NombreEmpleada, a.last_name AS ApellidoEmpleada, b.city AS CiudadSupervisor,b.reports_to AS IdSupervisor, b.first_name AS NombreJefe,  b.last_name AS ApellidoJefe
...
SELECT ord.order_id AS IDPedido, cu.company_name AS NombreEmpresa, ord.order_date AS FechaPedido
FROM orders AS ord
